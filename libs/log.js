...
function getSystemFromFile(file) {
    return logFileMap[file] || "general";
}

function logAttachFile(system) {
	var file = getErrorObject(3).filename;
    var idx = file.lastIndexOf("/");
    var fileStrip = file.slice(idx + 1);
    if (!logLevels[system]) {
        initializeLogSystem(system, 0);
    }
    logFileMap[fileStrip] = system;
    Debug.debug("Attached " + fileStrip + " to " + system, "log");
}
